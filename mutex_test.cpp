...
static void vector_thread_worker(std::vector<std::string>& to_append, const std::string& string_to_append, size_t index) {
	{
...
void test_vector_filling_with_mutex ()
{
	...
	if((test_vector.size() == 12 * 100)) {
		...
		std::cout << console_colors::red << "BAD SIZE (" << test_vector.size() << ")" << console_colors::remove_all_colors << std::endl;
	}
}
